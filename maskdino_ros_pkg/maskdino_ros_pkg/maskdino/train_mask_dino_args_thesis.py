...
DATASET_FILENAME_TIMBERSEG = '/data_private/data/timberseg/labels.json'          # COCO style annotation file
IMAGE_DIR_TIMBERSEG = '/data_private/data/timberseg/images'
...
USE_TLDD = False           # Use tldd dataset
...
def init_mask_dino(config_file, output_dir, fold_num=None, initial_weights=None): 
    ...
    if USE_TLDD:
        ...
        cfg.DATASETS.TRAIN = ("tldd_train", "timberseg")
        #cfg.DATASETS.TEST = ("tldd_val",)
        cfg.SOLVER.MAX_ITER = int(1090/3 * 10)
    else:
        cfg.DATASETS.TRAIN = ("timberseg",)
        #cfg.DATASETS.TEST = ("timberseg",)
        cfg.SOLVER.MAX_ITER = int(220/3 * 10)


    DatasetCatalog.register("timberseg", lambda view=dataset_timberseg: get_fiftyone_dicts(view))
    MetadataCatalog.get("timberseg").set(thing_classes=['logs'], evaluator_type="coco")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--use_tldd', action='store_true', help='Use the TLDD dataset')
    ...
    dataset_timberseg = fo.Dataset.from_dir(
            dataset_type=fo.types.COCODetectionDataset,
            data_path=IMAGE_DIR_TIMBERSEG,
            labels_path=DATASET_FILENAME_TIMBERSEG,
        )
